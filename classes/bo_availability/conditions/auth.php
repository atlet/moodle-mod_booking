 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class auth implements bo_condition {

    /** @var int $id Standard Conditions have hardcoded ids. */
    public $id = BO_COND_AUTH;
    ...
    public function is_available(booking_option_settings $settings, $userid, $not = false):bool {
        ...
        $isavailable = true;
        $data = $DB->get_record('booking', ['id' => $settings->bookingid], 'auth', IGNORE_MISSING);
    ...
    private function get_description_string($isavailable, $full) {
        if ($isavailable) {
            ...
            $description = $full ? get_string('bo_cond_auth_not_available', 'mod_booking') :
                get_string('bo_cond_auth_not_available', 'mod_booking');
        }
        return $description;
    }
}
